...
(`status --change`/`--next`/`--graph`, `prompt --skill`/`--list-skills`, `verify --requirements`,
`doctor --verbose`, `new-change --type definition`, `analyze --maturity`, `verify --strict`). Every command's

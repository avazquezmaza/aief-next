...
| `aief verify --change <id>` | The selected Change, its `manifest.harness`/`manifest.loop` (if any), `<changeDir>/loop.md` (if any) | Nothing, unless `manifest.harness.log` is `true` (appends `<changeDir>/hooks.md`) or `manifest.loop.verify` is declared (appends `<changeDir>/loop.md`) | Structural Verification for one Change; says exactly which one. Same Harness disabled/failure-visibility treatment as `aief prompt` (Change 0056/ADR-026). With `loop.verify` configured, also prints a "Loop:" summary (attempt N of M, retry available/limit reached) — Change 0057/ADR-027. Neither ever affects PASS/FAIL or the exit code. |

...
These are orthogonal concepts. An architect can work on a `basic` Change; a junior developer can work on a `migration` one. Two meanings for one word in the ubiquitous language ([domain-model.md](../../concepts.md)) is exactly the degradation `external-harness-patterns.md §5.8` warned about when rejecting imported terminology — and here AIEF would be doing it to itself.

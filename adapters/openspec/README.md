...
| Real OpenSpec release | **Validated against an installed release (2026-08-31): OpenSpec 1.5.0.** `openspec --help`'s command list contains no `propose` — confirmed with `grep -i propose` against the real output (no match) — matching the documentation-only expectation above exactly. Running `aief propose "add login"` against this real binary produces the documented fallback message verbatim (`OpenSpec 1.5.0 is installed but does not expose a "propose" command. Falling back to local Change generation.`) and creates a correct local `changes/<id>/proposal.md`. The nearest terminal equivalent this release actually exposes is `openspec new change <name>` (under `openspec new`, not top-level) — AIEF does not call it automatically; a project wanting OpenSpec-native change creation should run it directly, still driven by the human/assistant, matching "AIEF does not reimplement proposal generation" above. |
